...
create table if not exists public.payments (
  ...
  invoice_status text not null default 'pendiente' check (invoice_status in ('facturado', 'pendiente', 'emitiendo')),
...
create table if not exists public.invoices (
  ...
  description text not null,
  amount numeric(14, 2) not null check (amount > 0),
  currency_id text not null default 'PES',
...
create table if not exists public.user_arca_credentials (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  arca_environment text not null default 'production'
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (user_id, arca_environment)
);
